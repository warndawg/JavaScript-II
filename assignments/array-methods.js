...
let allCaps = [];
runners.map( r =>allCaps.push(r.first_name.toUpperCase()));
console.log(allCaps); 
...
// Problem 1 email list to send out newsletters 
let emailList = [];
runners.forEach(r => emailList.push(r.email));
console.log(emailList);

// Problem 2 shout out to all the companies represented
let shoutOutComs = [];
runners.forEach(r => shoutOutComs.push(r.company_name));
console.log("Lets give a shout out to the compenies represented " + shoutOutComs);

// Problem 3 free photo to all runners that donnated 100 dolars or more
let photo = [];
photo.push(
    runners.filter((d)=>{
        return d.donation >= 100;    
    })
);
console.log(photo);